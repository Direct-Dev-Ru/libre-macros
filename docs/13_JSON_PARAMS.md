...
| `<<Переменные.…>>` | В `find`/`replace`, лямбдах, **путях/масках «Файлы-Источники»**: подстановка из карты; в `'…'` (find/replace) — литерал без подстановки. Карта при старте сбора уже содержит глобальные, `__ENV_*` (среда) и `__OS_*` (ОС) — см. [04_DATA_COLLECTION.md](04_DATA_COLLECTION.md) §6.0.1 |
