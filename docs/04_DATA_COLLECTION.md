...
### 6.0.1 Автозагрузка `__ENV_*` и `__OS_*` в runtime-карту

При **каждом** запуске сбора (`collect_workbooks` / `collect_pack`) в сегмент
«Глобальные_переменные» автоматически попадают:

| Префикс | Источник | Пример плейсхолдера |
|---------|----------|---------------------|
| `__ENV_` | все переменные среды процесса (`os.environ`) | `<<Переменные.__ENV_HOME>>`, `<<Переменные.__ENV_PATH>>` |
| `__OS_` | сведения об ОС / runtime (stdlib; память — `/proc` / Windows API / `psutil` если есть) | `<<Переменные.__OS_USER>>`, `<<Переменные.__OS_HOME>>` |

Типовые `__OS_*`:

| Имя | Смысл |
|-----|--------|
| `__OS_USER` / `__OS_USERNAME` | пользователь ОС |
| `__OS_HOME` | домашний каталог |
| `__OS_CWD` | текущий каталог процесса |
| `__OS_TEMP` / `__OS_TMPDIR` | каталог временных файлов |
| `__OS_PLATFORM` / `__OS_SYSTEM` / `__OS_RELEASE` / `__OS_VERSION` | платформа |
| `__OS_MACHINE` / `__OS_PROCESSOR` / `__OS_ARCHITECTURE` | архитектура / CPU |
| `__OS_NODE` / `__OS_HOSTNAME` | имя узла |
| `__OS_PYTHON_VERSION` | версия Python |
| `__OS_PID` / `__OS_CPU_COUNT` | PID / число CPU |
| `__OS_UID` / `__OS_GID` | Unix uid/gid (если доступны) |
| `__OS_MEMORY_TOTAL_MB` / `__OS_MEMORY_AVAILABLE_MB` | ОЗУ (если удалось определить) |
| `__OS_SUMMARY` | краткая сводка одной строкой |

Ключ в карте: `Имя~Глобальные_переменные~Глобальные_переменные` (как у пользовательских глобальных).  
Символ `~` в имени env заменяется на `_`. Порядок seed: настройки глобальных → `__ENV_*`/`__OS_*` → ручной ввод (после `merge_run_context_begin`).

**Безопасность:** в карту попадает **весь** `os.environ` (в т.ч. секреты, токены). Не логируйте карту целиком и не подставляйте `__ENV_*` в отчёты без необходимости. См. [19_SECURITY.md](19_SECURITY.md).

Код: `libre_macros_runtime_env_lib.py`; вызов из `_merge_seed_globals_into_variables_map` / `merge_run_context_begin`.
